"""Text extraction from industrial documents — PDF, DOCX, CSV, XLSX, PPTX, HTML, EML, images."""
...
def _pdf(path: Path) -> tuple[str, list[dict]]:
    try:
        ...
        if len(full_text.strip()) < 30:
            ...
            if ocr_text:
                pages = [{"page": i + 1, "text": t} for i, t in enumerate(ocr_text)]
                full_text = "\n\n".join(ocr_text)
        return full_text, pages
    except Exception as e:
        return f"PDF extraction failed: {e}", [{"page": 1, "text": ""}]
...
def _ocr_pdf(path: Path) -> list[str]:
    try:
        ...
        return [pytesseract.image_to_string(img) for img in convert_from_path(str(path), dpi=180)]
    except Exception:
        return []
...
def _docx(path: Path) -> tuple[str, list[dict]]:
    try:
        ...
        return f"DOCX extraction failed: {e}", [{"page": 1, "text": ""}]
...
def _image(path: Path) -> tuple[str, list[dict]]:
    try:
        ...
        return f"Image OCR failed: {e}", [{"page": 1, "text": ""}]
...
def extract_entities(text: str) -> dict:
    lowered = text.lower()
    asset_tags = {
        _normalize_asset(v) for v in ASSET_PATTERN.findall(text)
        if _normalize_asset(v).split("-", 1)[0] not in {"IR", "SOP", "OEM", "ISO", "API"}
    }
    return {
